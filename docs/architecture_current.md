...
| **15f** | Wire dry-run port into AssistedReplyService behind flags | ✅ | [phase15f_done.md](phase15f_done.md) |
| **15g** | Assisted dashboard action endpoint planning (docs) | ✅ | [phase15g_done.md](phase15g_done.md) |
| **15h** | Live PDD AssistedOutboundPort planning (docs) | ✅ | [phase15h_done.md](phase15h_done.md) |
...
| **Productization** | 商家 SaaS + product gate + **PreviewReplyLogService** | **14l–15l** · action routes flag-gated ✅ · live send **未实现** |
...
| approve/reject/send on Dashboard | 📋 **15g planned** · **not implemented** |
...
| 组件 | 状态 |
|------|------|
| `AssistedReplyService` | ✅ skeleton + **15f dry-run wire** |
...
| `AssistedReplyService` wire-in | ✅ **15f** dry-run only · live **未实现** |
...
**AssistedReplyService Dry-run Wire（15f · implemented）：**

| 组件 | 状态 |
|------|------|
| `approve_pending` dry-run path | ✅ flags + allowlist |
| `DryRunAssistedOutboundPort` | ✅ would_send only |
| idempotency acquire | ✅ · no mark_succeeded on dry-run |
| pending → sent | ❌ |
| live send | ❌ **`live_send_not_implemented`** when dry_run=false |

| 文档 | 内容 |
|------|------|
| [phase15f_done.md](phase15f_done.md) | Dry-run wire · no live send |

**Dashboard Action Endpoints（15g planned · 15i skeleton · 15l registration）：**

| 组件 | 状态 |
|------|------|
| `pending_assisted_action_routes.py` | ✅ POST approve/reject skeleton |
| POST approve / reject routes | ✅ **15i** · **15l flag-gated register** |
| `PRODUCT_DASHBOARD_ACTION_ROUTES_ENABLED` | ✅ default **false** |
| `apply_dashboard_action_route_bootstrap()` | ✅ called from `app.py` · no-op when flag off |
| Calls AssistedReplyService only | ✅ |
| CSRF + confirm + viewer RBAC | ✅ route-level |
| `dry_run_expected=true` required | ✅ |
| First impl dry-run only | ✅ |
| live send | ❌ **未实现** |
| handler / SendMessage / PDD | ❌ **未改** |

| 文档 | 内容 |
|------|------|
| [phase15g_dashboard_action_endpoint_plan.md](phase15g_dashboard_action_endpoint_plan.md) | 总体规划 |
| [phase15g_approve_endpoint_contract.md](phase15g_approve_endpoint_contract.md) | Approve contract |
| [phase15g_reject_endpoint_contract.md](phase15g_reject_endpoint_contract.md) | Reject contract |
| [phase15g_permissions_csrf_idempotency.md](phase15g_permissions_csrf_idempotency.md) | 权限 · CSRF |
...
| [phase15g_done.md](phase15g_done.md) | 15g 签收 |
| [phase15i_done.md](phase15i_done.md) | 15i 签收 · I1–I18 |
| [phase15l_done.md](phase15l_done.md) | 15l 签收 · L1–L15 |

**Dashboard Action Idempotency（15j · skeleton implemented）：**

| 组件 | 状态 |
|------|------|
| `DashboardActionIdempotencyRow` | ✅ |
| `ActionIdempotencyRepositorySQLite` | ✅ acquire / complete / fail |
| Route `client_request_id` integration | ✅ behind flag |
| Replay same payload | ✅ return stored response |
| Different payload conflict | ✅ 409 |
| live send | ❌ **未实现** |
| app.py registration | ✅ **15l** bootstrap · **default off** |

| 文档 | 内容 |
|------|------|
| [phase15j_done.md](phase15j_done.md) | 15j 签收 |

**Dashboard Action Route Registration（15l · implemented）：**

| 组件 | 状态 |
|------|------|
| `PRODUCT_DASHBOARD_ACTION_ROUTES_ENABLED` | ✅ default **false** |
| `apply_dashboard_action_route_bootstrap()` | ✅ in `app.py` startup |
| Idempotent `register_pending_assisted_action_routes` | ✅ |
| live send | ❌ **未实现** |
...
**Live PDD Port Skeleton（15k · planning-only）：**

| 组件 | 状态 |
|------|------|
| `LivePddAssistedOutboundPort` class location | 📋 `live_pdd_assisted_outbound_port.py` |
| Implements `AssistedOutboundPort` | 📋 documented |
...
| [phase15k_pdd_send_primitive_boundary.md](phase15k_pdd_send_primitive_boundary.md) | Primitive |
| [phase15k_result_mapping_contract.md](phase15k_result_mapping_contract.md) | Result mapping |
| [phase15k_safety_flags_and_allowlist.md](phase15k_safety_flags_and_allowlist.md) | Flags |
| [phase15k_no_fallback_and_hot_path_boundary.md](phase15k_no_fallback_and_hot_path_boundary.md) | No fallback |
...
| [phase15h_done.md](phase15h_done.md) | 签收 |

**Live Assisted Send Integration（15e · planning-only）：**

| 组件 | 状态 |
|------|------|
| AssistedReplyService → idempotency + port wiring | ✅ **15f** dry-run · live **15h+** |
| live assisted send | ❌ **未实现** |
| auto send | ❌ **未实现** |
...
**Productization（12a–15l）：** **15l** Dashboard action route flag-gated registration（default off · dry-run only）；**15m** live port impl 待做。
