...
| **Productization** | 商家 SaaS + product gate + **PreviewReplyLogService** | **14l–14u** SQLite · Policy schema ✅ · **Final Guard** planned · send **未实现** |
...
**Dashboard read API（14o · skeleton implemented）：**

| 组件 | 状态 |
|------|------|
| `DashboardReadService` | ✅ list + detail · read-only |
| `api_read_routes.py` | ✅ GET skeleton · no-op register on PyQt app |
...
| approve/reject / final guard / assisted send | ❌ **未实现** · [phase14r_done.md](phase14r_done.md) planned |
| auto send | ❌ **未实现** |

**AssistedReplyService（14r · planning-only）：**

| 组件 | 状态 |
|------|------|
| `AssistedReplyService` stub | ✅ `NotImplementedError` |
| create_pending / approve / reject / expire | 📋 planned · 14x+ skeleton |
| final guard integration | ✅ **14t** planned · **14v** impl · **not implemented** |
...
| template validation / forbidden scan | ❌ **未实现** · `validation_status` field only |
...
**Final Guard + Merchant Policy（14t · planning-only）：**

| 组件 | 状态 |
|------|------|
| `evaluate_final_guard` pure function | 📋 planned · **未实现** |
| G1–G27 rule matrix | 📋 documented |
| forbidden promise scan | 📋 documented |
| I/O contract | 📋 documented |
| audit/snapshot integration | 📋 documented |
| assisted / auto send | ❌ **未实现** |
...
**Productization（12a–14u）：** **14u** MerchantSafetyPolicy + MerchantReplyTemplate schema/repository skeleton behind flags；**14v** Final Guard impl 待做。
...
| **14v** | Final Guard pure function implementation | [phase14u_done.md](phase14u_done.md) |
| **14w** | Policy/template validation service skeleton | [phase14u_done.md](phase14u_done.md) |
| **14x** | Assisted service skeleton behind flags | [phase14u_done.md](phase14u_done.md) |
