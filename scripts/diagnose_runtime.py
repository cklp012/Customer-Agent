...
def main() -> int:
    ...
    print("  Phase 9d: registry path defaults ON when env is unset (create_pinduoduo_registry_channel")
    print("  respects wrapper on/off). Roll back: USE_CHANNEL_REGISTRY_FOR_AUTOREPLY=false.")
    print("  python app.py bootstraps PINDUODUO before AutoReply; expect registry in app process.")
    print("  This diagnose script is a separate process (no bootstrap): registry_fallback is normal.")
    print("  Unregistered / create failure in app still falls back to _create_auto_reply_legacy.")
    print()
