...
def main() -> int:
    ...
    print("  Phase 9b: when registry flag is true and PINDUODUO is registered, AutoReply may")
    print("  use ChannelRegistry.create for wrapper on OR off; the registry factory")
    print("  (create_pinduoduo_registry_channel) respects USE_PINDUODUO_CHANNEL_WRAPPER.")
    print("  Unregistered registry / create failure falls back to _create_auto_reply_legacy.")
    print("  create_pinduoduo_channel remains wrapper-only for direct calls.")
    print("  Phase 9c: production default is still legacy_factory (registry flag unset).")
    print("  Gray-test registry path: USE_CHANNEL_REGISTRY_FOR_AUTOREPLY=true in app env.")
    print("  Default-on migration is planned for Phase 9d, not 9c.")
    print()
