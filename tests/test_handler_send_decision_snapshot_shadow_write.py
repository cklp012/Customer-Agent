"""Phase 14n: handler + SendDecision snapshot shadow write integration tests."""
...
class TestHandlerSendDecisionSnapshotShadowWrite(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self._tmpdir = _REPO_ROOT / "temp" / "phase14n_handler_tests"
        self._tmpdir.mkdir(parents=True, exist_ok=True)
        self._db_path = self._tmpdir / "product_gate_handler_snapshot.db"
    ...
    def test_test_shop_flags_on_zero_send_with_snapshot_row(
        self,
    ...
    @patch.object(AIReplyHandler, "_get_ai_reply", new_callable=AsyncMock, return_value=_AI_REPLY)
    @patch(
        "product_persistence.repositories.sqlite_send_decision_repository.SendDecisionRepositorySQLite.create_snapshot",
        side_effect=RuntimeError("snapshot down"),
    )
    @patch("Channel.pinduoduo.utils.API.send_message.SendMessage.send_text")
    def test_snapshot_write_failure_no_send_in_memory_retained(
        self,
        send_text_mock: MagicMock,
    ...
    @patch.object(AIReplyHandler, "_send_reply", new_callable=AsyncMock, return_value=True)
    @patch.object(AIReplyHandler, "_get_ai_reply", new_callable=AsyncMock, return_value=_AI_REPLY)
    def test_non_test_shop_flags_on_legacy_no_snapshot(
        self,
        _ai_mock: AsyncMock,
        send_mock: AsyncMock,
    ) -> None:
    ...
    @patch.object(AIReplyHandler, "_send_reply", new_callable=AsyncMock, return_value=True)
    @patch.object(AIReplyHandler, "_get_ai_reply", new_callable=AsyncMock, return_value=_AI_REPLY)
    def test_doudian_flags_on_no_snapshot(
        self,
        _ai_mock: AsyncMock,
        send_mock: AsyncMock,
    ) -> None:
    ...
    def test_handler_no_snapshot_repository_imports(self) -> None:
        source = _HANDLER_SOURCE.read_text(encoding="utf-8")
        self.assertNotIn("sqlite_send_decision_repository", source)
